// ============================================
//   WORD CRASH GAME MODULE — Solo + Multi
...
function buildSetupEmbed(game) {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      ...
      `**🎯 Solo:** اضغط SOLO باش تلعب وحدك\n` +
      `**👥 Multi:** اضغط JOIN + START\n\n` +
      `**📋 How to play:**\n` +
      `• البوت يعطيك كلمة **مبعثرة الحروف**\n` +
      `• لازم ترتّبها صح\n` +
      `• أول واحد يجاوب صح → **+1 نقطة**\n` +
      `• **${TOTAL_ROUNDS} كلمات** في اللعبة`
    )
    .addFields({
      name: "👥 Players Joined",
      value: playersList,
...
function buildGameEmbed(game) {
  ...
  return new EmbedBuilder()
    .setColor(COLOR_PLAY)
    .setTitle(game.solo ? "🎮 كلمات كراش — Solo" : "🎮 كلمات كراش")
    .setDescription(
      `**Category:** 🎯 كلمة\n\n` +
...
function buildWinnerEmbed(game) {
  ...
  return new EmbedBuilder()
    .setColor(COLOR_WIN)
    .setTitle(game.solo ? "🏆 كلمات كراش — انتهت!" : "🏆 كلمات كراش — Winner!")
    .setDescription(
      (game.solo
        ? `## 🎉 خلصت اللعبة!\n\n**نتيجتك:** \`${winner.score}/${game.totalRounds}\`\n\n`
