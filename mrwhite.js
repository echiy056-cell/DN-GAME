...
const QUESTIONS_TIME = 5 * 60 * 1000;   // ✅ 5:00 minutes total (كان 3)
const QUESTION_TURN_TIME = 60;           // ✅ 60s per player turn (كان 30)
...
// ============================================
//   ❓ QUESTIONS PANEL (Turn-based, 60s each)
// ============================================
...
// ============================================
//   🎭 MR WHITE REVEAL PANEL (public, with button)
// ============================================
...
// ============================================
//   🎯 GUESS PANEL (ephemeral, only Mr. White)
// ============================================
function buildGuessContainer(game, options) {
  ...
  // 7 boutons — نقسموهم على 2 rows (4 + 3)
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      if (id.startsWith("mrwhite_start_")) {
        ...
        // 🕵️ Mr. White = عشوائي بين كل اللاعبين
      ...
      // ===== OPEN GUESS (Mr. White clicks in channel) =====
      ...
      // ===== GUESS (Mr. White chooses in ephemeral) =====
...
// ============================================
//   ❓ START QUESTIONS PHASE (5:00 min total, 60s per turn)
// ============================================
async function startQuestionsPhase(client, game) {
  try {
    ...
    console.log(`❓ Questions phase started (${game.questionsSecondsLeft}s total, ${QUESTION_TURN_TIME}s per turn)`);

    // نبداو الـtotal timer (5 دقائق)
    ...
    // نبداو الـturn timer (60s للاعب الأول)
...
async function processVotes(client, game) {
  try {
    ...
    if (eliminated === game.mrWhiteId) {
      ...
      // ✅ نجهزو الـguess options — 7 كلمات (6 غالطين + 1 صحيح)
      const wrongWords = WORDS
        .filter(w => w.word !== game.word)
        .sort(() => Math.random() - 0.5)
        .slice(0, 6);   // 6 كلمات غالطين

      const correctWord = WORDS.find(w => w.word === game.word);
      const options = [...wrongWords, correctWord].sort(() => Math.random() - 0.5);  // 7 كلمات

      game.guessOptions = options;

      // نبعثو reveal panel في الشات العام مع bouton
