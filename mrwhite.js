...
const DESCRIBE_TIME = 3 * 60 * 1000;   // 3:00 minutes
const QUESTIONS_TIME = 3 * 60 * 1000;  // 3:00 minutes
const QUESTION_TURN_TIME = 30 * 1000;  // 30s per player turn
...
// ============================================
//   🎭 DESCRIBE PANEL (3:00 min — VC)
// ============================================
function buildDescribeContainer(game, secondsLeft) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent("## 🎭 Describe Phase")
  );
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      `**🎙️ Join the voice channel and describe the word!**\n\n` +
      `Each player must describe the word **without saying it directly**.\n` +
      `Mr. White must blend in!\n\n` +
      `**⏱️ Time left:** \`${timeStr}\``
    )
  );
      ...
      if (id.startsWith("mrwhite_start_")) {
        ...
        game.countdownInterval = setInterval(async () => {
          ...
          if (secondsLeft <= 0) {
            ...
            startDescribePhase(client, game);
            return;
          }
...
// ============================================
//   🎙️ START DESCRIBE PHASE (3:00 min)
// ============================================
async function startDescribePhase(client, game) {
  try {
    game.phase = "describe";
    ...
    let secondsLeft = DESCRIBE_TIME / 1000;

    const container = buildDescribeContainer(game, secondsLeft);
    ...
    console.log(`🎙️ Describe phase started (${secondsLeft}s)`);

    game.describeInterval = setInterval(async () => {
      secondsLeft--;

      if (secondsLeft <= 0) {
        clearInterval(game.describeInterval);
        game.describeInterval = null;
        startQuestionsPhase(client, game);
        return;
      }

      try {
        const c = buildDescribeContainer(game, secondsLeft);
        const msg = await game.channel.messages.fetch(game.messageId);
        await msg.edit({
          components: [c],
          flags: MessageFlags.IsComponentsV2,
    ...
    console.error("❌ Error in startDescribePhase:", err);
  }
}
...
async function startQuestionsPhase(client, game) {
  try {
    ...
    // نمسحو panel الـdescribe
    try {
      const msg = await game.channel.messages.fetch(game.messageId);
      await msg.delete();
    } catch {}

    let totalSecondsLeft = QUESTIONS_TIME / 1000;

    // نبعثو أول panel
    const currentPlayer = game.players[game.currentTurnIndex];
    const container = buildQuestionContainer(game, currentPlayer, 30, totalSecondsLeft);
    const newMsg = await game.channel.send({
      components: [container],
      flags: MessageFlags.IsComponentsV2,
    });
    game.messageId = newMsg.id;
    game.turnMessageId = newMsg.id;

    console.log(`❓ Questions phase started (${totalSecondsLeft}s total)`);

    // نبداو الـtotal timer (3 دقائق)
    game.questionsInterval = setInterval(async () => {
      totalSecondsLeft--;
      ...
      // نحدّثو الـpanel بالوقت الكلي
      try {
        const currentP = game.players[game.currentTurnIndex];
        const c = buildQuestionContainer(game, currentP, game.turnSecondsLeft || 30, totalSecondsLeft);
        const msg = await game.channel.messages.fetch(game.messageId);
        await msg.edit({
          components: [c],
          flags: MessageFlags.IsComponentsV2,
...
async function sendNewQuestionPanel(client, game) {
  ...
  // نحسبو الوقت الكلي الباقي
  const totalSecondsLeft = game.questionsSecondsLeft || QUESTIONS_TIME / 1000;
