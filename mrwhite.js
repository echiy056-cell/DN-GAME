...
const QUESTIONS_TIME = 5 * 60 * 1000;
const QUESTION_TURN_TIME = 60;
const VOTE_TIME = 30 * 1000;
const GUESS_TIME = 30 * 1000;
const VC_CHECK_INTERVAL = 10 * 1000;  // كل 10s نفحصو VC
...
// ✅ Check كل اللاعبين في VC
function getPlayersNotInVoice(guild, players) {
  const notInVoice = [];
...
function buildRevealContainer(game, eliminated) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      `**🎯 FINAL CHANCE:**\n` +
      `Mr. White, click the button below to guess the word!\n\n` +
      `**⏱️ Time:** \`30 seconds\``
    )
  );
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`mrwhite_openguess_${game.id}`)
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      if (message.content.startsWith(`${PREFIX}mrwhite`) ||
        ...
        // ✅ Check VC — الـhost لازم في VC
        if (!isInVoice(message.member)) {
          return message.reply("❌ You must be in a **voice channel** to play Mr. White!");
        }
        ...
        const game = {
          ...
          guild: message.guild,             // ✅ نحتاجو للـVC check
          players: [],
          votes: {},
          eliminated: [],
          phase: "setup",
          ...
          turnTimer: null,
          turnInterval: null,
          turnSecondsLeft: QUESTION_TURN_TIME,
          ...
          currentTurnIndex: 0,
          vcCheckInterval: null,            // ✅ للـVC check
          warnedPlayers: [],                // ✅ لتفادي التكرار
        };
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      if (id.startsWith("mrwhite_join_")) {
        ...
        // ✅ Check VC — اللي كبس JOIN لازم في VC
        ...
        if (game.players.length >= MAX_PLAYERS) {
          return interaction.reply({ content: `❌ Game full (${MAX_PLAYERS} players max).`, ephemeral: true });
        }
      ...
      if (id.startsWith("mrwhite_start_")) {
        ...
        const notInVC = getPlayersNotInVoice(interaction.guild, game.players);
        ...
        console.log(`🕵️ Mr. White = ${game.players[randomIndex].username}`);
      ...
      // ===== OPEN GUESS =====
      if (id.startsWith("mrwhite_openguess_")) {
        const gameId = id.replace("mrwhite_openguess_", "");
        const game = mrwhiteGames.get(gameId);
        ...
        const guessContainer = buildGuessContainer(game, game.guessOptions);
      ...
      if (id.startsWith("mrwhite_guess_")) {
        ...
        // ✅ نحيّو الـVC check
        if (game.vcCheckInterval) clearInterval(game.vcCheckInterval);
...
async function startQuestionsPhase(client, game) {
  try {
    ...
    // ✅ نبداو VC check (كل 10s)
    game.warnedPlayers = [];
    game.vcCheckInterval = setInterval(async () => {
      if (game.phase !== "questions" && game.phase !== "vote") return;

      try {
        await game.guild.members.fetch();
        const notInVC = getPlayersNotInVoice(game.guild, game.players);

        for (const p of notInVC) {
          if (!game.warnedPlayers.includes(p.id)) {
            game.warnedPlayers.push(p.id);
            try {
              await game.channel.send({
                content: `⚠️ <@${p.id}> left the voice channel!`,
              });
            } catch {}
          }
        }
