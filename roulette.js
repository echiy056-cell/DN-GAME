...
const SPIN_DELAY = 10 * 1000;         // 10s before spin
const ELIMINATION_TIME = 30 * 1000;   // 30s to choose
...
async function generateWheelImage(players) {
  try {
    ...
    // مساحة إضافية للأسماء تحت
    const playerRowHeight = 120;
    const rows = Math.ceil(players.length / 5); // 5 لاعبين في السطر
    ...
    // نرسمو خلفية سوداء شفافة للأسماء
...
function buildEliminationContainer(game, chooser) {
  ...
  // Boutons بأسماء اللاعبين (ماشي اللاعب نفسه)
  const buttons = [];
  game.players.forEach((p) => {
    if (p.id === chooser.id) return; // ما ينجمش يطيّر روحو
    buttons.push(
      new ButtonBuilder()
...
async function spinWheel(client, game) {
  try {
    ...
    // نبعثو panel الـelimination (ephemeral)
    try {
      ...
      // إلا فشل الـDM، نبعثو في الشات العام مع mention
      try {
        const container = buildEliminationContainer(game, chooser);
