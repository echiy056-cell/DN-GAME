// ============================================
//   ROULETTE GAME MODULE — مع Canvas + Buttons
...
const PICK_MEMBER_TIMEOUT = 5 * 1000;   // 5 ثواني قبل اختيار عضو
const CHOOSE_TIMEOUT      = 7 * 1000;   // 7 ثواني للاختيار ✅
...
async function drawWheelImage(players) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      const avatarImg = await loadImage(players[i].avatar);
...
// ============================================
//   🎯 PICK PANEL — مع username في الأزرار
// ============================================
function buildPickContainer(game, chosenPlayer) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      ...
      `**<@${chosenPlayer.id}>**, choose who to eliminate!\n\n` +
...
async function startPickPhase(client, game) {
  try {
    ...
    // ⏱️ 7 ثواني — كان ما اختارش، البوت يختار
