...
/**
 * 🧹 يمسح كل اللعبات القديمة في القناة
 */
function clearOldGames(channelId) {
  ...
  if (count > 0) console.log(`🧹 Cleared ${count} old game(s) in channel`);
}

/**
 * 🔍 يلقى آخر لعبة (الأحدث) في القناة
 */
  ...
  client.on("interactionCreate", async (interaction) => {
    try {
      ...
      if (id === "trivia_start") {
        ...
        // 🧹 نمسحو اللعبات القديمة في نفس القناة
      ...
      if (id.startsWith("trivia_ans_")) {
        ...
        if (isCorrect) {
          ...
          console.log("❌ Wrong answer! Sending lose panel...");
          ...
          try {
            ...
            console.log("❌ Lose panel sent!");
          } catch (err) {
            console.error("❌ Error sending lose panel:", err);
          }
      ...
      if (id === "trivia_next") {
        ...
        // نمسحو اللعبة القديمة ونعملو وحدة جديدة
        if (game.questionTimer) clearTimeout(game.questionTimer);
        if (game.questionCountdown) clearInterval(game.questionCountdown);
...
async function nextQuestion(client, game) {
  try {
    ...
    game.questionTimer = setTimeout(async () => {
      ...
      // Time's up → Lose
      console.log("⏱️ Time's up! Sending lose panel...");
      game.answered = true;
      game.streak = 0;
      ...
      try {
        ...
        console.log("❌ Time-out lose panel sent!");
      } catch (err) {
        console.error("❌ Error sending timeout lose panel:", err);
      }
