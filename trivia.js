// ============================================
//   TRIVIA QUIZ — V2 + Solo + 3 Power-ups
...
const HELP_MENTION_TIME = 40 * 1000;
const HELP_ANSWER_TIME = 10 * 1000;
...
function buildPowerupRow(game) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setLabel(game.powerups.help ? "🆘 Help" : "❌ Help")
      .setStyle(ButtonStyle.Secondary)
...
//   ✅ ANSWER REVEAL
// ============================================
function buildAnswerContainer(game, isCorrect, revealAll = true) {
  const q = game.currentQuestion;
  const container = new ContainerBuilder().setAccentColor(
...
// ============================================
//   🆘 HELP PANEL (Step 1: Ask for mention)
// ============================================
function buildHelpMentionContainer(game, secondsLeft) {
  const container = new ContainerBuilder().setAccentColor(COLOR_HELP);

  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent("## 🆘 Help from my friend")
  );
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      `**اكتب منشن @صاحبك في الشات**\n` +
      `*(Ex: @Ali_Pro)*\n\n` +
      `⏱️ **عندك:** \`${secondsLeft}s\``
    )
  );
...
// ============================================
//   🆘 HELP PANEL (Step 2: Friend's turn)
// ============================================
function buildHelpFriendContainer(game, friendId, friendUsername) {
  const q = game.currentQuestion;

  const container = new ContainerBuilder().setAccentColor(COLOR_HELP);

  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent("## 🆘 Help Request")
  );
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      ...
      `**⏱️ عندك:** \`${HELP_ANSWER_TIME / 1000}s\`\n\n` +
      `**السؤال:**\n# ${q.q}`
    )
  );
...
// ============================================
function buildHelpAnswerContainer(game, friendId, secondsLeft) {
  const q = game.currentQuestion;

  const container = new ContainerBuilder().setAccentColor(COLOR_HELP);

  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent("## 🆘 Friend Answer")
  );
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      `<@${friendId}> اختر الجواب!\n\n` +
      `⏱️ **⏱️ عندك:** \`${secondsLeft}s\`\n\n` +
      `# ${q.q}`
    )
  );
...
function buildWinContainer(game) {
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      `🎉 **ربحت اللعبة كاملة!**\n\n` +
      `🎯 **النقاط:** \`${game.score}\`\n` +
      `🔥 **أفضل سلسلة:** \`${game.bestStreak}\`\n` +
      `✅ **أجوبة صحيحة:** \`${game.correctCount}/${TOTAL_QUESTIONS}\`\n\n` +
      `### هل تريد أن تكمل اللعب؟`
    )
  );
...
//   ❌ LOSE CONTAINER
// ============================================
function buildLoseContainer(game, correctIdx) {
  const q = game.currentQuestion;

  const container = new ContainerBuilder().setAccentColor(COLOR_LOSE);

  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent("## ❌ لقد أجبت بالخطأ")
  );
  ...
  container.addTextDisplayComponents(
    new TextDisplayBuilder().setContent(
      `**الجواب الصحيح:** # ${q.o[correctIdx]}\n\n` +
      `🎯 **النقاط:** \`${game.score}\`\n` +
      `🎮 **السؤال:** \`${game.round}/${TOTAL_QUESTIONS}\`\n\n` +
      `### إذا أردت المحاولة مرة أخرى انقر على زر إعادة`
    )
  );
...
function init(client) {
  ...
  client.on("messageCreate", async (message) => {
    try {
      ...
      // ===== @mention للـ Help =====
      for (const [gameId, game] of triviaGames.entries()) {
        if (game.phase !== "helping_mention") continue;
        if (game.channel.id !== message.channel.id) continue;
        if (message.author.id !== game.playerId) continue;

        const mention = message.mentions.users.first();
        if (!mention) continue;
        if (mention.bot) continue;
        if (mention.id === game.playerId) continue;

        // ✅ لقينا العضو
        game.helpFriendId = mention.id;
        game.helpFriendUsername = mention.username;
        game.phase = "helping_friend";

        // نوقف الـ 40 timer
        if (game.helpMentionTimer) clearTimeout(game.helpMentionTimer);

        // نمسحو رسالة الـ mention
        try { await message.delete(); } catch {}

        // نمسحو بانال الـ "اكتب mention"
        try {
          const m = await game.channel.messages.fetch(game.messageId);
          await m.delete();
        } catch {}

        // نبعتو بانال الـ Help للعضو
        const helpContainer = buildHelpFriendContainer(game, mention.id, mention.username);
    ...
    try {
      ...
      if (id === "trivia_start") {
        ...
        const game = {
          ...
          startTime: 0,
          hiddenAnswers: [],
          powerups: { help: true, fifty: true, vote: true },
          questionTimer: null,
          questionCountdown: null,
          helpMentionTimer: null,
          helpAnswerTimer: null,
          helpFriendId: null,
          helpFriendUsername: null,
        };
      ...
      if (id.startsWith("trivia_ans_")) {
        ...
        const game = findGameByChannel(interaction.channel.id);
        ...
        if (isCorrect) {
          ...
          // نمسحو البانال ونبعتو بانال الخسارة
          try {
            const m = await game.channel.messages.fetch(game.messageId);
            await m.delete();
          } catch {}
          ...
          const loseMsg = await game.channel.send({
            ...
            components: [buildLoseContainer(game, game.correctIdx)],
            flags: MessageFlags.IsComponentsV2,
          });
      ...
      if (id === "trivia_help") {
        ...
        if (!game) return interaction.reply({ content: "❌ No active game.", ephemeral: true });
        if (interaction.user.id !== game.playerId) return interaction.reply({ content: "❌ Not your game.", ephemeral: true });
        if (!game.powerups.help) return interaction.reply({ content: "❌ Already used!", ephemeral: true });
        if (game.answered) return interaction.reply({ content: "❌ Already answered!", ephemeral: true });
        ...
        // نبعتو بانال "اكتب mention"
        const helpMsg = await game.channel.send({
          content: `<@${game.playerId}>`,
          components: [buildHelpMentionContainer(game, HELP_MENTION_TIME / 1000)],
          flags: MessageFlags.IsComponentsV2,
        });
        game.messageId = helpMsg.id;

        // ⏱️ 40s للـ mention
        game.helpMentionTimer = setTimeout(async () => {
          if (game.phase === "helping_mention") {
            await handleHelpTimeout(client, game);
          }
        }, HELP_MENTION_TIME);
        return;
      ...
      if (id === "trivia_help_accept") {
        const game = findGameByChannel(interaction.channel.id);
        if (!game) return interaction.reply({ content: "❌ No active game.", ephemeral: true });
        if (interaction.user.id !== game.helpFriendId) return interaction.reply({ content: "❌ Not your request.", ephemeral: true });

        if (game.helpAnswerTimer) clearTimeout(game.helpAnswerTimer);
        game.phase = "helping_answer";

        const container = buildHelpAnswerContainer(game, interaction.user.id, HELP_ANSWER_TIME / 1000);
      ...
      if (id.startsWith("trivia_help_ans_")) {
        ...
        const answerContainer = buildAnswerContainer(game, isCorrect);

        try {
          await interaction.message.delete();
        } catch {}

        const resultMsg = await game.channel.send({
          content: `<@${game.playerId}>`,
          components: [answerContainer],
          flags: MessageFlags.IsComponentsV2,
        });
        game.messageId = resultMsg.id;

        // كان السؤال 10 → ربح
        if (isCorrect && game.round === TOTAL_QUESTIONS) {
          setTimeout(async () => {
            game.score += FINAL_QUESTION_BONUS;
            const user2 = getUser(game.playerId);
            user2.points += FINAL_QUESTION_BONUS;
      ...
      if (id === "trivia_next") {
        ...
        game.answered = false;
        game.hiddenAnswers = [];
        game.powerups = { help: true, fifty: true, vote: true };
        game.phase = "play";
...
async function nextQuestion(client, game) {
  try {
    ...
    console.log(`🧠 Q${game.round}/${TOTAL_QUESTIONS}: ${q.q}`);
