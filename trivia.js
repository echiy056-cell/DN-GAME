...
// ============================================
//   🔀 SHUFFLE — الإجابة الصحيحة في مكان عشوائي
// ============================================
function shuffleQuestion(q) {
  // نعملو مصفوفة [0, 1, 2, 3] ونخلطوها
  const indices = [0, 1, 2, 3].sort(() => Math.random() - 0.5);
  
  // نرتبو الخيارات حسب الترتيب الجديد
  const shuffledOptions = indices.map(i => q.o[i]);
  
  // نلقاو وين راحت الإجابة الصحيحة
  const newCorrectIdx = indices.indexOf(q.c);
  
  console.log(`🔀 Shuffle: old=${q.c} new=${newCorrectIdx} | options: ${shuffledOptions.join(" | ")}`);
...
async function nextQuestion(client, game) {
  try {
    ...
    console.log(`🧠 Q${game.round}/${TOTAL_QUESTIONS} | Correct: ${["A", "B", "C", "D"][game.correctIdx]} | ${q.q}`);
